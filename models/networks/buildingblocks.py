...
def _add_conditioned_section(nodes, downsampling_level, num_blocks, cond, coupling_type, act_norm, permutation_type):
    """
    Original conditional block from https://github.com/jleuschn/cinn_for_imaging/blob/master/cinn_for_imaging/reconstructors/networks/cond_net_mri.py
    ...
    for k in range(num_blocks):
        if k % 2 == 0:
            ...
            subnet = subnetworks.subnet_conv3x3
...
def _add_conditioned_section_new(nodes, downsampling_level, num_blocks, cond, coupling_type, act_norm, permutation_type, down_layer=0):
    """
    ...
    for k in range(num_blocks):
        ...
        if act_norm:
            nodes.append(Ff.Node(nodes[-1].out0, Fm.ActNorm, {}, name="ActNorm_{}.{}.{}".format(downsampling_level, k, down_layer)))
